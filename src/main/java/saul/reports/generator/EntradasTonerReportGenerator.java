...
/**
 * Generador de reportes PDF para entradas de tóner.
 * Crea reportes con encabezado estándar del hospital y tabla de entradas de tóner.
 */
@Component
@PropertySource("classpath:reports.properties")
public class EntradasTonerReportGenerator {
    ...
    @Value("${report.title.entradas.toner}")
    ...
    public EntradasTonerReportGenerator(PDFBoxService pdfBoxService) {
        this.pdfBoxService = pdfBoxService;
    }

    /**
     * Genera un reporte PDF con las entradas de tóner.
     *
     * @param entradas lista de entradas de tóner a incluir en el reporte
     ...
     */
    public byte[] generateReport(List<EntradasToner> entradas, LocalDate fechaInicio, LocalDate fechaFin, Long idUsuario) throws IOException {
        try (PDDocument document = new PDDocument();
            ...
            try (PDFPaginator paginator = new PDFPaginator(document, pageConfig)) {
                ...
                if (entradas != null && !entradas.isEmpty()) {
                    drawEntradasTable(paginator, entradas);
                } else {
    ...
    private void drawQRCode(PDFPaginator paginator) throws IOException {
        float qrSize = 60f; // Reducido un 10%

        QRCodeConfig qrConfig = QRCodeConfig.medium()
                .size((int) qrSize)
                .margin(0);

        // Para posicionar el QR a la derecha, necesitamos usar el método específico
        pdfBoxService.insertQRCodeRight(paginator, qrText, qrConfig);
        paginator.addSpace(15f); // Reducido al 50%
    }
    ...
    private void drawEntradasTable(PDFPaginator paginator, List<EntradasToner> entradas) throws IOException {
        // Preparar datos de la tabla
        List<List<String>> tableData = new ArrayList<>();

        // Agregar encabezados
        List<String> headers = List.of("ID", "Fecha Entrada", "Usuario", "Tipo Tóner", "Cantidad", "Observaciones");
        tableData.add(headers);

        // Convertir entradas a filas de datos
        for (EntradasToner entrada : entradas) {
            List<String> row = List.of(
                entrada.getId().toString(),
                entrada.getFechaEntrada().format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm")),
                entrada.getUsuarioEntrada() != null ? entrada.getUsuarioEntrada().getNombreCompleto() : "",
                entrada.getTipoToner() != null ? entrada.getTipoToner().getNombreTipoToner() : "",
                entrada.getCantidad().toString(),
                entrada.getObservaciones() != null ? entrada.getObservaciones() : ""
            );
            tableData.add(row);
        }

        // Configuración de tabla con anchos de columna proporcionales (autoajustables)
        // Columnas: ID(0.8), Fecha(1.5), Usuario(1.8), TipoToner(1.8), Cantidad(1), Observaciones(2.5)
        TableConfig tableConfig = TableConfig.minimal()
                .width(paginator.getUsableWidth())
                .columnWidthRatios(0.8f, 1.5f, 1.8f, 1.8f, 1f, 2.5f)
                .rowHeight(22f)
                .padding(4f)
                .fontSize(9f)
                .headerFontSize(10f)
    ...
    private void drawNoDataMessage(PDFPaginator paginator) throws IOException {
        ...
        pdfBoxService.writeText(paginator, "No se encontraron entradas de tóner en el período especificado.", noDataConfig);
    }
