...
/**
 * Generador de reportes PDF para tickets.
 * Crea reportes en orientación horizontal con encabezado estándar del hospital
 * y tabla de tickets con estilo dotted en color negro.
 */
@Component
@PropertySource("classpath:reports.properties")
public class TicketReportGenerator {
    ...
    @Value("${report.title.tickets}")
    private String reportTitle;

    @Value("${report.qr.text1}")
    private String qrText;

    // Color negro para bordes de tabla (dotted)
    private static final Color TABLE_BORDER_COLOR = Color.BLACK;
    ...
    public TicketReportGenerator(PDFBoxService pdfBoxService) {
        this.pdfBoxService = pdfBoxService;
    }

    /**
     * Genera un reporte PDF con los tickets filtrados.
     *
     ...
     */
    public byte[] generateReport(List<TicketReportDTO> tickets, LocalDate fechaInicio, LocalDate fechaFin, String filtros) throws IOException {
        try (PDDocument document = new PDDocument();
            ...
            try (PDFPaginator paginator = new PDFPaginator(document, pageConfig)) {
                ...
                if (tickets != null && !tickets.isEmpty()) {
                    drawTicketsTable(paginator, tickets);
                } else {
    ...
    private void drawFiltersInfo(PDFPaginator paginator, LocalDate fechaInicio, LocalDate fechaFin, String filtros) throws IOException {
        ...
        filterInfo.append("Período de creación: ").append(fechaInicio.format(DATE_FORMATTER))
    ...
    private void drawTicketsTable(PDFPaginator paginator, List<TicketReportDTO> tickets) throws IOException {
        // Preparar datos de la tabla
        List<List<String>> tableData = new ArrayList<>();

        // Agregar encabezados (Descripción en lugar de Creador)
        List<String> headers = List.of("ID", "Asunto", "Departamento", "Estado", "Prioridad", "Descripción", "Asignado", "F. Creación", "F. Cierre");
        tableData.add(headers);

        // Agregar filas de datos
        for (TicketReportDTO t : tickets) {
            // Formatear fechas
            String fechaCreacion = t.getFechaCreacion() != null ? t.getFechaCreacion().format(DATETIME_FORMATTER) : "-";
            String fechaCierre = t.getFechaCierre() != null ? t.getFechaCierre().format(DATETIME_FORMATTER) : "-";

            List<String> row = List.of(
                String.valueOf(t.getIdTicket()),
                t.getAsunto() != null ? t.getAsunto() : "-",
                t.getDepartamento() != null ? t.getDepartamento() : "-",
                t.getEstadoTicket() != null ? t.getEstadoTicket() : "-",
                t.getPrioridad() != null ? t.getPrioridad() : "-",
                t.getDescripcion() != null ? t.getDescripcion() : "-",
                t.getUsuarioAsignado() != null ? t.getUsuarioAsignado() : "-",
                fechaCreacion,
                fechaCierre
            );
            tableData.add(row);
        ...
        // Configuración de tabla con estilo dotted y color negro
        // Anchos proporcionales: ID(1), Asunto(2.5), Depto(2), Estado(1.5), Prioridad(1.5), Descripción(3), Asignado(2), F.Creación(2), F.Cierre(2)
        TableConfig tableConfig = TableConfig.minimal()
                .width(paginator.getUsableWidth())
                .columnWidthRatios(1f, 2.5f, 2f, 1.5f, 1.5f, 3f, 2f, 2f, 2f)
                .rowHeight(20f)
                .padding(4f)
                .fontSize(8f)
                .headerFontSize(9f)
                ...
                .border(TableConfig.BorderStyle.DOTTED, 1.0f, TABLE_BORDER_COLOR);
    ...
    private void drawNoDataMessage(PDFPaginator paginator) throws IOException {
        ...
        pdfBoxService.writeText(paginator, "No se encontraron tickets para los filtros especificados.", noDataConfig);
    }
